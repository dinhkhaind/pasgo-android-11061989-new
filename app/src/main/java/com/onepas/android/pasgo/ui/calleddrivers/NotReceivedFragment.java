...
public class NotReceivedFragment extends BaseFragment {
	private View mRoot;
	...
	private void getDanhSachDaNhan() {
		String url = WebServiceUtils
				.URL_LICH_SU_DAT_TRUOC_CHUA_NHAN(Pasgo.getInstance().token);
	...
	private void setListAdapter(ArrayList<DatTruoc> datTruocs) {
		...
		if (sizeList > 0) {
			...
			CalledDriverAdapter adapter = new CalledDriverAdapter(getActivity(),datTruocs,false);
